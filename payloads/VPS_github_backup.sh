...
#
echo "[Starting to secure copy database backup to the server...]"
...
then
    ...
    # Get the GitHub reponame to define the remote server destination folder
...
echo "[VPS_deploy has finished sending database backup to remote server...]"
#
...
then
    ...
    # For each repository listed in github_userdata file
    while read -r -a githubuser
    do
      # Eliminate comments
      if [[ ${githubuser[0]:0:1} != "#" && ! -z "${githubuser[0]}" ]]; then
